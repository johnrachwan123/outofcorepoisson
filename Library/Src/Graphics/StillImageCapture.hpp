/***************************************************************************************************
 * File:         $URL: https://svn.bolitho.us/Library/Trunk/Src/Graphics/StillImageCapture.hpp $
...
namespace Bolitho
{
  namespace Graphics
  {
    class LIBRARY_API StillImageCapture : public Object
    {
      DeclareRuntimeType();
    public:
      /* Constructs a still image capture context for a device */
      StillImageCapture(LPCTSTR DeviceName, UINT Width, UINT Height);
      
      /* Begins raw continous capture mode */
      VOID BeginRawCapture(BOOL Accumulate);

      /* Begins a capture sequence */
      VOID BeginRawCapture(UINT SettleFrameCount, UINT CaptureFrameCount, BOOL Accumulate);

      /* Waits for a capture sequence to complete */
      VOID Capture(UINT SettleFrameCount, UINT CaptureFrameCount, BOOL Accumulate);
      ...
      /* Returns the last raw frame.  The format is 24bpp RGB. */
      Buffer<BYTE> GetLastRawFrame();

      /* Returns the accumulation buffer */
      Buffer<FLOAT> GetAccumulationBuffer();
    ...
    private:
      ...
      {
      public:
        Callback(StillImageCapture& C) : m_Capture(C) 
        {}

        /* IUnknown */
        virtual HRESULT STDMETHODCALLTYPE QueryInterface(CONST IID& InterfaceId, VOID** ppInterface)
        { return E_NOTIMPL; }
        ...
        /* ISampleGrabberCB */
        virtual HRESULT STDMETHODCALLTYPE SampleCB(DOUBLE, IMediaSample*)
        { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE BufferCB(DOUBLE, BYTE*, LONG);
      
      private:
        StillImageCapture& m_Capture;
      };
